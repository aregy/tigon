...
using (PdfDocumentProcessor documentProcessor = new PdfDocumentProcessor()) {
    ...
    documentProcessor.LoadDocument(@"fw9 filled unsigned.pdf");

    for (var i = documentProcessor.Document.Pages.Count; i > 1; i--){
	documentProcessor.DeletePage(i);
    }
    X509Certificate2 certificate = new X509Certificate2(@"pfx.pfx", "", X509KeyStorageFlags.MachineKeySet);
    ...
    PdfOrientedRectangle signatureBounds = new PdfOrientedRectangle(new PdfPoint(128, 250), 83, 20, angleInRadians);
    ...
    documentProcessor.SaveDocument(@"SignedDocument2.pdf", new PdfSaveOptions() { Signature = signature });
}
