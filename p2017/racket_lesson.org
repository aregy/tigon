...
#+begin_src scheme :tangle summary.rkt
...
In the simple case using immutable hash tables to generate a dictionary of frequencies given an input list:
#+begin_src scheme :tangle functions.rkt
  (define/contract (lst-freq1 lst)
    (-> list? hash?)
    (for/fold ([acc (hash)])
	([x lst])
...
Optimizing that a bit more by making use of the fact that has-ref support a third argument (the default value reported when no key exists):

#+begin_src scheme :tangle functions.rkt
  (define/contract (lst-freq2 lst)
    (-> list? hash?)
    (for/fold ([acc (hash)])
	([x lst])
...
Analogous to using for-each in place of for we can use foldl in place of for/fold to elimate the explicit assigment of our index to a range [x lst]:

#+begin_src scheme :tangle functions.rkt
  (define/contract (lst-freq3 lst)
    (-> list? hash?)
    (foldl (lambda (x h)
	     (hash-set h x (add1 (hash-ref h x 0))))
	   (hash)
	   lst))
#+end_src
